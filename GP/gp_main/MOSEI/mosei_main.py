...
def train_net(X_train_emb, X_train_vedio, X_train_audio, y_train, X_valid_emb, X_valid_vedio,
    ...
    def evaluate(model, X_valid_emb, X_valid_vedio, X_valid_audio, y_valid, criterion, batchsize=64):
        ...
        model.eval()
        with torch.no_grad():
            total_n = X_valid_emb.shape[0]
            num_batches = int(total_n / batchsize) + 1
            for batch in range(num_batches):
                start = batch * batchsize
                end = (batch + 1) * batchsize
                batch_X_embed = torch.Tensor(X_valid_emb[start:end, :, :]).cuda()
                batch_X_v = torch.Tensor(X_valid_vedio[start:end, :, :]).cuda()
                batch_X_a = torch.Tensor(X_valid_audio[start:end, :, :]).cuda()
                batch_y = torch.LongTensor(y_valid[start:end]).cuda()
                predictions = model.forward(batch_X_embed, batch_X_v, batch_X_a)
                loss = criterion(predictions, batch_y)
                epoch_loss += loss.item()
        return epoch_loss / num_batches
    ...
    def predict(model,X_test_emb, X_test_vedio, X_test_audio, batchsize=64):
        ...
        model.eval()
        with torch.no_grad():
            # total_n = X_test.shape[1]#####0?1
            total_n = X_test_emb.shape[0]
            num_batches = int(total_n / batchsize) + 1
            for batch in range(num_batches):
                start = batch * batchsize
                end = (batch + 1) * batchsize
                batch_X_embed = torch.Tensor(X_test_emb[start:end, :, :]).cuda()
                batch_X_v = torch.Tensor(X_test_vedio[start:end, :, :]).cuda()
                batch_X_a = torch.Tensor(X_test_audio[start:end, :, :]).cuda()
                predictions = model.forward(batch_X_embed, batch_X_v, batch_X_a)
                # print("predictions shape111aaa:",predictions.shape)
                predictions = F.softmax(predictions, 1)
                # print("predictions shape2bbb:", predictions.shape)
                predictions = predictions.cpu().data.numpy()
                # print("predictions shape3ccc:", predictions.shape)
    ...
    for epoch in range(config["num_epochs"]):
        ...
        if valid_loss <= best_valid:
            ...
            model_name = './model_save/mosei/mosei_model_{}.pt'.format(epoch)
            torch.save(model, model_name)
        else:
            print(epoch, train_loss, valid_loss)
            model_name = './model_save/mosei/mosei_model_1_{}.pt'.format(epoch)
            torch.save(model, model_name)

    model = torch.load('./model_save/main1_2/mosei_model_1_0.pt', map_location='cuda')
